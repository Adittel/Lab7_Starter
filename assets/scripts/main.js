...
function bindEscKey() {
  ...
  // Reference: https://developer.mozilla.org/en-US/docs/Web/API/Document/keydown_event
   ...
   function logKey(e) { if(e.keyCode == 27) router.navigate('home'); }
}
...
function bindPopstate() {
  ...
  // Reference: https://developer.mozilla.org/en-US/docs/Web/API/Window/popstate_event
   window.addEventListener('popstate', logKey, true);
   function logKey(e) {
    console.log("location: " + document.location + ", state: " + JSON.stringify(event.state));
    let x=String(document.location);

    if(x.includes("#"))
      router.navigate(x.split('#')[1], true)
